...
def extract_1d_spec(standard_star=False, show_reidentify_QA=False):
    ...
    ID_init = dict(peak=pixnumber, wavelength=wavelength)
    ...
    for i in range(0, N_REID - 1):
        ...
        for peak_pix_init in ID_init["pixel_gauss"]:
            ...
            g_init = Gaussian1D(
                ...
                mean=mean_init,
                stddev=stddev_init,
                bounds={"amplitude": (0, 2 * np.max(cropped)), "stddev": (0, TOL_REID)},
            )
            const = Const1D(amplitude=np.mean(cropped))
            g_model = g_init + const
            g_fit = fitter(g_model, x_cropped, cropped)
            fit_center = g_fit.mean_0.value
            # Calculate the full width half max (FWHM)
            fitted_FWHM = g_fit.stddev_0.value * 2.355
            if (
                abs(fit_center - peak_pix_init) > TOL_REID
                or fitted_FWHM > 3 * FWHM_ID
                or fitted_FWHM < 0.5 * FWHM_ID
                or g_fit.amplitude_0.value < 1
            ):  # flat line cases:

                if show_reidentify_QA:
                    plt.plot(x_cropped, cropped, "o")
                    plt.plot(x_cropped, g_fit(x_cropped))
                    plt.title(
                        "Initial: {:.2f}, Fit: {:.2f},\n FWHM_INIT: {:.2f}, FWHM_FOUND: {:.2f}".format(
                            peak_pix_init, fit_center, FWHM_ID, fitted_FWHM
                        )
                        + "\nRejected"
                    )
                    plt.show()
        ...
        if FITTING_MODEL_ID.lower() == "chebyshev":
            coeff_REID1D, fitfull = chebfit(
                peak_gauss_REID_nonan,
            ...
            raise ValueError(
                "Function {:s} is not implemented.".format(FITTING_MODEL_REID)
            )

        print(
            "[{:04d}:{:04d}]\t{:d}/{:d}\t{:.3f}".format(
                lower_cut, upper_cut, n_found, n_tot, fitRMS
            )
        )

        points = np.vstack((line_REID.flatten(), np.tile(spatialcoord, len(line_REID))))
    ...
    if FITTING_MODEL_REID.lower() == "chebyshev":
        coeff_init = Chebyshev2D(
            x_degree=ORDER_WAVELEN_REID, y_degree=ORDER_SPATIAL_REID
        )
    ...
    ax1.plot(points[:, 0], points[:, 1], ls="", marker="+", color="r", alpha=0.8, ms=10)
    ...
    while nregions <= 4:
        tpoints = plt.ginput(
            n=1, timeout=30, show_clicks=True, mouse_add=1, mouse_stop=2
        )
        pix_ref, _ = tpoints[0]
    ...
    # Interactive check
    x_sky = np.hstack(
        (np.arange(ap_sky[0], ap_sky[1]), np.arange(ap_sky[2], ap_sky[3]))
    )
    sky_val = np.hstack(
        (apall_1[ap_sky[0] : ap_sky[1]], apall_1[ap_sky[2] : ap_sky[3]])
    )
    ...
    if FITTING_MODEL_APSKY.lower() == "chebyshev":
        ...
        ax.legend()
        ax.set_title(
            title_str.format(FITTING_MODEL_APSKY, ORDER_APSKY, SIGMA_APSKY, ITERS_APSKY)
        )
        ax.set_xlabel("Pixel number")
    ...
    for i in range(N_AP - 1):
        ...
        sky_val = np.hstack(
            (apall_i[ap_sky[0] : ap_sky[1]], apall_i[ap_sky[2] : ap_sky[3]])
        )

        # Subtract fitted sky
        if FITTING_MODEL_APSKY.lower() == "chebyshev":
            # TODO: maybe we can put smoothing function as IRAF APALL's b_naverage
            clip_mask = sigma_clip(
                sky_val, sigma=SIGMA_APSKY, maxiters=ITERS_APSKY
            ).mask
            coeff, fitfull = chebfit(
                x_sky[~clip_mask], sky_val[~clip_mask], deg=ORDER_APSKY, full=True
            )
            ...
            raise ValueError(
                "Function {:s} is not implemented.".format(FITTING_MODEL_APSKY)
            )
        ...
        peak_pix = peak_local_max(cropped, min_distance=FWHM_AP, num_peaks=1)
        if len(peak_pix) == 0:
        ...
        # TODO: put something like "lost_factor" to multiply to FWHM_ID in the bounds.
        g_init = Gaussian1D(
            amplitude=cropped[peak_pix],
            mean=peak_pix,
            stddev=FWHM_AP * gaussian_fwhm_to_sigma,
            bounds={
                "amplitude": (0, 2 * cropped[peak_pix]),
    ...
    resid_mask = sigma_clip(
        aptrace - chebval(x_aptrace, coeff_aptrace),
        sigma=SIGMA_APTRACE,
        maxiters=ITERS_APTRACE,
    ).mask

    # width
    coeff_aptrace_fwhm = chebfit(x_aptrace, aptrace_fwhm, deg=ORDER_APTRACE)
    resid_mask_fwhm = sigma_clip(
        aptrace_fwhm - chebval(x_aptrace, coeff_aptrace_fwhm),
        sigma=SIGMA_APTRACE,
        maxiters=ITERS_APTRACE,
    ).mask
    ...
    coeff_aptrace_fwhm_fin = chebfit(
        x_aptrace_fwhm_fin, aptrace_fwhm_fin, deg=ORDER_APTRACE
    )
    ...
    title_str = (
        "Aperture Trace Fit ({:s} order {:d})\n"
        + "Residuials {:.1f}-sigma, {:d}-iters clipped"
    )
    plt.suptitle(
        title_str.format(
            FITTING_MODEL_APTRACE, ORDER_APTRACE, SIGMA_APTRACE, ITERS_APTRACE
        )
    )
    ...
    ax2.axhline(
        -np.std(resid_aptrace_fin, ddof=1), ls=":", color="k", label="residual std"
    )
    ax3.plot(x_aptrace, aptrace_fwhm, ls="", marker="+", ms=10)
    ax3.plot(
        x_aptrace_fwhm_fin,
        fit_aptrace_fwhm_fin,
        ls="--",
    ...
    for i in range(N_WAVELEN):
        ...
        x_sky = np.hstack(
            (np.arange(ap_sky_i[0], ap_sky_i[1]), np.arange(ap_sky_i[2], ap_sky_i[3]))
        )
        sky_val = np.hstack(
            (cut_i[ap_sky_i[0] : ap_sky_i[1]], cut_i[ap_sky_i[2] : ap_sky_i[3]])
        )
        clip_mask = sigma_clip(sky_val, sigma=SIGMA_APSKY, maxiters=ITERS_APSKY).mask
    ...
    hdr.add_history(
        f"Sky subtracted using sky offset = {ap_sky_offset}, "
    ...
    s = chebval(x_ap, coeff_aptrace_fwhm_fin) / np.sqrt(8.0 * np.log(2.0))
    ...
    fig, axs = plt.subplots(
        2, 1, figsize=(10, 6), sharex=False, sharey=False, gridspec_kw=None
    )
    if not standard_star:
    ...
    dw = np.float32((ap_wavelen_end - ap_wavelen_start) / (float(N_WAVELEN) - 1.0))
    ...
    out3 = f(wavelength)
    f = interp1d(
        ap_wavelen, np.sqrt(var_optimal), fill_value="extrapolate", kind="cubic"
    )
    out4 = f(wavelength)
    ...
    multispecdata = fake_multispec_data(
        (np.float32(out1), np.float32(out2), np.float32(out3), np.float32(out4))
    )
    ...
    hduout.header["CRPIX1"] = 1.0
    hduout.header["CCDSUM"] = "1 2     "
    hduout.header["WCSDIM"] = 3
    hduout.header["CD1_1"] = dw
    hduout.header["CD2_2"] = 1.0
    hduout.header["LTM1_1"] = 1.0
    hduout.header["LTM2_2"] = 1.0
    ...
    hduout.header["CD3_3"] = 1.0
    hduout.header["LTM3_3"] = 1.0
    ...
    plt.plot(wavelength, out2, lw=1, alpha=0.5, color="r", label="1d summed spectrum")
    plt.plot(
        wavelength,
        out1,
        lw=1,
        alpha=0.75,
        label="1d Horne-extracted spectrum",
        color="b",
    )
