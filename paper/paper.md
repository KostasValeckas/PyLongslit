...
or by summing detector counts for a box-like object shape [@photutils], which can be useful for emission-line-dominated objects.


# Pipeline

The figures below describe the pipeline structure. The pipeline 
architecture is inspired by the very popular (but no longer maintained) IRAF [@IRAF]. In a broad sense, there
...
The data and instructions needed to reproduce the PyLongslit results can be found [here](https://kostasvaleckas.github.io/PyLongslit/getting_started.html).
The raw data and all pipeline output for the PypeIt calculations can be downloaded [here](https://1drv.ms/u/c/1f8eedcff5109e73/IQCDsH7Kb7-rSKGVdDq4NcWQASfXO-LmNjDqQuR2lP80Zs4?e=WfeR6K). Instructions on how to re-create the results using the raw data can be found in the [PypeIt documentation](https://pypeit.readthedocs.io/en/stable/). All the input needed by the pipeline (such as the `.pypeit` files) are provided together with the raw data.
...
instrument independent. Due to these design choices, the software does not account for any instrument-specific phenomena, such as detector fringing and the like. The software will likely be less precise than an instrument-specific pipeline (depending on the implementation of the latter). The code 
