...
optflux = objopt / 10 ** (
    (chebval(ID_init["wavelength"], coeff_ID) - airmass * extinterp1d) / 2.5
)
sumflux = objsum / 10 ** (
    (chebval(ID_init["wavelength"], coeff_ID) - airmass * extinterp1d) / 2.5
)
...
plt.plot(wavelength, optflux, lw=1, alpha=1.0, label="flux-calibrated 1d spectrum")
