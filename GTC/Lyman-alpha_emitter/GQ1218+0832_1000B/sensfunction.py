...
ID_init = dict(wavelength=wlstd, conversion=C)
ID_init = Table(ID_init)

if FITTING_MODEL_SF.lower() == "chebyshev":
    coeff_ID, fitfull = chebfit(
        ID_init["wavelength"], ID_init["conversion"], deg=ORDER_SF, full=True
    )
    #    fitRMS = np.sqrt(fitfull[0][0]/len(ID_init))
    residual = ID_init["conversion"] - chebval(ID_init["wavelength"], coeff_ID)
...
ax1.plot(
    ID_init["wavelength"], ID_init["conversion"], ls=":", color="k", ms=10, marker="+"
)
ax2.plot(ID_init["wavelength"], residual, ls="", color="k", ms=10, marker="+")
