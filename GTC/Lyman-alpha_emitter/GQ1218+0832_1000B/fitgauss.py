...
image_noise = image_data * 0.0 + np.std(image_data[58:97, :])

plt.figure()
plt.imshow(
    image_data, cmap="gray_r", vmin=vmin, vmax=vmax, origin="lower", aspect="auto"
)
plt.colorbar()
...
width_x : %.1f
width_y : %.1f"""
    % (y + xstar - fws, x + ystar - fws, width_x, width_y),
    fontsize=16,
...
plt.show()
wl = 10.0 * ((y + xstar - fws + 11099 - 100.0) * hdr["CD1_1"] + hdr["CRVAL1"])
...
left = np.mean(image_data[:, 0 : 11045 - 11099 + 100], axis=1) * 1.0e19
...
right = np.mean(image_data[:, 11134 - 11099 + 100 : 200], axis=1) * 1.0e19
