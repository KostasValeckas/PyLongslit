...
for i in range(0, N_REID - 1):
    ...
    reidentify_i = np.sum(stdlampimage[lower_cut:upper_cut, :], axis=0)
    peak_gauss_REID = []
...
for i in (1, 2, 3):
    ...
    ax2.plot(
        hcutax, fit2D_REID(hcutax, hcutrep), lw=1, label="hcut {:d}".format(int(hcut))
    )

    vcut_profile = fit2D_REID(vcutrep, vcutax)
    vcut_normalize = vcut_profile - np.median(vcut_profile)
    ax3.plot(vcut_normalize, vcutax, lw=1, label="vcut {:d}".format(int(vcut)))
...
x_sky = np.hstack((np.arange(ap_sky[0], ap_sky[1]), np.arange(ap_sky[2], ap_sky[3])))
sky_val = np.hstack((apall_1[ap_sky[0] : ap_sky[1]], apall_1[ap_sky[2] : ap_sky[3]]))
...
plt.suptitle(
    title_str.format(FITTING_MODEL_APTRACE, ORDER_APTRACE, SIGMA_APTRACE, ITERS_APTRACE)
)
...
ax2.axhline(
    -np.std(resid_aptrace_fin, ddof=1), ls=":", color="k", label="residual std.dev."
)
...
ax3.set_ylim(0.3 * np.mean(fit_aptrace_fwhm_fin), 2.0 * np.mean(fit_aptrace_fwhm_fin))
ax2.legend()
plt.show()
...
for n in range(N_WAVELEN):
    ...
    ap_optimal[n] = np.sum(data_skysub[:, n] * weight / data_variance[:, n]) / np.sum(
        weight**2 / data_variance[:, n]
    )
...
hduout.header.extend(
    hdrcopy, strip=True, update=True, update_first=False, useblanks=True, bottom=False
)

hduout.header["HISTORY"] = "From development of extract_std.py"
...
plt.plot(
    wavelength, out1, lw=1, alpha=0.75, label="1d Horne-extracted spectrum", color="b"
)
