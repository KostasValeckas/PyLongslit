...
std_ref_data = np.loadtxt(stdnm + ".dat")
...
plt.plot(lam, stdcounts, lw=1, alpha=0.5, label="1d extracted standard star spectrum")
plt.legend()
...
for n in range(0, len(reflam)):
    ...
    if (wl > np.amin(lam)) & (wl < np.amax(lam)):
        ...
        plt.plot(
            [wl - 0.5 * bandwidth, wl - 0.5 * bandwidth],
            [minflux, maxflux],
            color="r",
            lw=1.5,
        )
...
print(
    "Double left-click on red boxes that should be deleted. Other mouse-clicks to exit."
)
deleted = list()
get_new_line = True
while get_new_line:
...
flam = refflux / 1.0e6 / 3.34e4 / reflam**2
