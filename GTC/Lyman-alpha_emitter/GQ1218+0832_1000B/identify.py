...
def create_pixtable(
    arc_image, grism_name, pixtable_name, linelist_fname, order_wl=4, app=None
):
    """
    arc_image : str
    ...
    if os.path.exists(output_pixtable) and gui.message == "ok":
        ...
        msg = (
            "          - Successfully saved line identifications: %s\n"
            % output_pixtable
        )
        ...
        msg = (
            " [ERROR]  - Something went wrong in line identification of %s\n"
            % grism_name
        )
        order_wl = None
...
class GraphicInterface(QMainWindow):
    ...
    def load_linelist_fname(self, linelist_fname=None):
        if linelist_fname is False:
            ...
            if self.first_time_open:
                print(
                    " [INFO] - Don't worry about the warning above. It's an OS warning that can not be suppressed."
                )
    ...
    def load_spectrum(self, arc_fname=None):
        ...
        if arc_fname:
            ...
            linelist_fname = os.path.join(calib_dir, "gtc.dat")
            self.load_linelist_fname(linelist_fname)
        ...
        if fname:
            with open(fname, "w") as tab_file:
                ...
                if np.sum(mask) < 2:
                    ...
                    tab_file.write(
                        "# Pixel Table for ALFOSC grism: %s\n" % self.grism_name
                    )
    ...
    def update_cache(self):
        if self.grism_name == "":
            QMessageBox.critical(
                None, "No grism name defined", "The grism name has not been defined."
            )
        msg = (
            "Are you sure you want to update the PyNOT pixel table for %s"
            % self.grism_name
        )
        messageBox = QMessageBox()
        messageBox.setText(msg)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "filename", type=str, nargs="?", default="", help="Raw arc-line spectrum"
    )
    parser.add_argument(
        "--lines",
        type=str,
    ...
    app = GraphicInterface(arc_fname, linelist_fname=linelist_fname, dispaxis=dispaxis)
    app.show()
    qapp.exec_()
